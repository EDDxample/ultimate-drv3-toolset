from translation_pipeline.src.utils import read, write, ensure_paths, padding
from translation_pipeline.src.utils import STEP_0_PATH, STEP_1_PATH, STEP_4_PATH
...
#      u4  name length
#      u2  padding
#          file name       (+ padding to 16 byte block)
#          compressed data (+ padding to 16 byte block)
...
def extract(filename):
    with open(f'{STEP_0_PATH}/{filename}.spc', 'rb') as obj:
        ...
        idk0        = read('u4', obj)
        padding(obj, 16)
        table_magic = read('string', obj, length=4)
        padding(obj, 12)

        for _ in range(file_count):
            compression_flag  = read('u2', obj)
            idk0              = read('u2', obj)
            compressed_size   = read('u4', obj)
            decompressed_size = read('u4', obj)
            name_length       = read('u4', obj) + 1 # null terminated
            padding(obj, 16)
            ...
            subfilename = read('string', obj, length=name_length - 1)
            ...
            ensure_paths(f'{STEP_1_PATH}/{filename}/{subfilename}')
            with open(f'{STEP_1_PATH}/{filename}/{subfilename}', 'wb') as current_file:
                current_file.write(file_data)
    
def repack(filename, subfiles):
    dataout =  bytes('CPS.', 'utf-8')
    dataout += b'\x00' * 36            # padding
    file_count = len(subfiles)
    dataout += write('u4', file_count)
    dataout += write('u4', 0)          # idk
    dataout += b'\x00' * 16            # padding
    dataout += bytes('Root', 'utf-8')
    dataout += b'\x00' * 12            # padding

    for subfilename in subfiles:
        file_data = open(f'{STEP_4_PATH}/{filename}/{subfilename}', 'rb').read()
        ...
        dataout += write('u4', comp_len)
        dataout += write('u4', raw_len)
        dataout += write('u4', name_len)
        dataout += b'\x00' * 16 # padding
...
def comp(data: bytes) -> bytes: 
    ...
    while True:
        ...
        if seq_len >= 2 and found_at != -1:
            ...
            block.extend(write('u2', repeat_data))
        else:
